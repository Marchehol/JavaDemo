...
public class TestServerSocket{
    public static void main(String args[]) throws Exception{
        ...
        编程一般都是死循环，永不休止地运行着。*/
        while(true){
            /*在服务器端调用accept()方法接受客户端的连接对象,accept()方法是一个阻塞式方法，
            一直在傻傻地等待着是否有客户端申请连接上来, 然后服务器端的Socket插座就和客户端的Socket插座建立了连接了*/
            Socket s = ss.accept();

            /*客户端能否连接上服务器端，取决于服务器端是否接受客户端的连接请求，如果接受了客户端的连接请求，
            那么在服务器端就安装上一个Socket插座，通过这个插座与连接上的客户端就可以建立连接，互相通信了*/
